...
try {
  ...
  // One list: the account's own login first, marked Main.
  const row = own.locator('.sftp-list li').first();
  await row.waitFor({ timeout: 30000 });
  check((await row.textContent()).includes(NAME) && await row.getByText('Main', { exact: true }).isVisible(), 'its own login heads the SFTP list, marked Main');
  check(await own.getByRole('button', { name: 'Turn SFTP off' }).count() === 0, 'which a user cannot turn off themselves');
  ...
  check(await form.getByRole('button', { name: 'Save' }).isDisabled(), 'the button waits for the current panel password');
  await form.getByLabel('Current panel password').fill('not the password at all');
  await form.getByRole('button', { name: 'Save' }).click();
  await own.waitForTimeout(1500);
  check(sftp(TYPED) && !sftp(OWN), 'a wrong panel password changes nothing');
  await form.getByLabel('Current panel password').fill(P3);
  await form.getByRole('button', { name: 'Save' }).click();
  await own.waitForTimeout(1500);
